...
- use the `extension.jar` generated by the `seed4j-extension` module workflow

Example setup for extension mode:

```bash
# build your extension project created with the seed4j-extension module
seed4j --version
seed4j list
```
...
In this case, rebuild the extension artifact using the `seed4j-extension` module workflow and replace `~/.config/seed4j-cli/runtime/active/extension.jar`.
