...
Inspect the registry and installed identity without copying a current version or SHA into long-lived documentation:
...
coordinate and declares no personal snapshot repository. `experimental` replaces those top-level authority values with
the personal coordinate, full upstream SHA, and unavailable-module facts, and adds its snapshot-only repository.
Ordinary Maven build and release commands package the identity already owned by the checked-out branch.
...
an expired snapshot may stop rebuilding. The publisher refresh policy makes a still-current snapshot eligible for
republication after 60 days, but it is not a durability guarantee. Prefer stable for long-lived or reproducible
projects.
...
4. the bot-authenticated build requests qualification and npm publication from trusted `main` workflow code.

Stable CLI changes travel separately from `main` to `experimental` through a checked synchronization PR. There is no
automatic wholesale `experimental` to `main` merge. A generally useful experimental adaptation needs its own
stable-focused PR to `main` and then flows forward again.
...
- an ephemeral GitHub App installation token authenticates the trusted checkout, disposable-branch push, and PR
  creation or update;
...
Because the App-created PR emits an ordinary pull-request event, the proposal receives its normal build. Synchronization
also explicitly dispatches `github-actions.yml` for the exact disposable-branch head, so the workflow has repairable,
immutable evidence even if a callback is missed. Finalization accepts only the supplied numeric run ID and revalidates
the run SHA, source repository, proposal branch, bot actor, completion, current source and target branches, exact
two-parent target/source topology, live PR head, and green `tests` result. Only current evidence enables auto-merge;
stale, pending, red, malformed, or conflicting evidence leaves the PR open or requests a refresh.

The bounded finalizer and 15-minute scheduled recovery complete asynchronous merges, repair a single exact partially
published proposal, dispatch a missing exact-head build once, finalize a reachable merge, and remove the disposable
branch and pending label in a retry-safe order. Recovery never fabricates evidence, guesses conflict resolution, or
enables merge without a current green review. See
...
snapshot registry. A snapshot PR auto-merges only after required checks pass on current `experimental`; an incompatible
update remains open and red for a maintainer adaptation. The snapshot version's 12-character SHA component must match
the separately recorded full upstream SHA.
...
with dist-tag `experimental` through npm Trusted Publishing and provenance. A documentation-only commit or any other
non-release-worthy commit completes without a new npm version. Experimental publication never creates or changes a
GitHub Release, Release Drafter draft, stable JAR asset, stable tag, or npm `latest` tag.
...
for cadence, token expiry, quota review, qualification, retention refresh, failure reporting, and retry behavior. Re-read
them before an operational change instead of treating this CLI repository as a second live configuration source.
...
and accepts no free-form repository, ref, SHA, coordinate, or version.

### Automatic versus maintainer intervention

| Situation                                                    | Normal handling                                                                                             |
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| Expected upstream-CI or recent-snapshot skip                 | Automatic; inspect the workflow summary only.                                                               |
| Retention refresh after 60 days                              | Automatic on the next eligible publisher run.                                                               |
| Clean `main` synchronization                                 | Automatic proposal, normal and dispatched builds, current-evidence review, auto-merge, and finalization.    |
...
| Release evaluation with no release-worthy commit             | Automatic successful completion without a new npm version.                                                  |
...
| Change from weekly to daily                                  | Maintainer records the at-most-80% quota review and changes the publisher configuration through a green PR. |
| Bad experimental npm version                                 | Maintainer follows the rollback procedure below from a trusted local npm session with 2FA.                  |
| Official replacement for the personal channel                | Maintainer follows the official-channel exit procedure below.                                               |
...
```bash
GOOD_VERSION='<last-good-experimental-version>'
BAD_VERSION='<bad-experimental-version>'
CURRENT_LATEST="$(npm view seed4j-cli dist-tags.latest)"
CURRENT_EXPERIMENTAL="$(npm view seed4j-cli dist-tags.experimental)"
ISSUE_URL='<public-seed4j-cli-issue-url>'
